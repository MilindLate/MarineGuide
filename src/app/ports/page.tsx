    ...
    <div className="p-8 max-w-6xl mx-auto space-y-6">
      <div className="flex items-center justify-between">
        <div className="space-y-1">
          <h1 className="text-2xl font-bold text-[#202124]">⚓ Port Status</h1>
          <p className="text-sm text-[#5f6368]">Real-time congestion monitoring — 50 major global ports</p>
        </div>
        <button className="flex items-center gap-2 px-4 py-2 border rounded-full text-xs font-bold text-[#5f6368] hover:bg-gray-50 transition-all">
        ...
        <table className="w-full text-sm">
          <thead className="bg-[#f8f9fa] border-b">
            <tr>
              ...
              <th className="px-6 py-4 text-left font-bold text-[#9aa0a6] text-[10px] uppercase tracking-wider">Port</th>
              <th className="px-6 py-4 text-left font-bold text-[#9aa0a6] text-[10px] uppercase tracking-wider">Region</th>
              <th className="px-6 py-4 text-left font-bold text-[#9aa0a6] text-[10px] uppercase tracking-wider">Congestion</th>
              <th className="px-6 py-4 text-left font-bold text-[#9aa0a6] text-[10px] uppercase tracking-wider">Vessels</th>
              <th className="px-6 py-4 text-left font-bold text-[#9aa0a6] text-[10px] uppercase tracking-wider">Avg Wait</th>
              <th className="px-6 py-4 text-left font-bold text-[#9aa0a6] text-[10px] uppercase tracking-wider">Risk Score</th>
            </tr>
          </thead>
          <tbody className="divide-y">
            {PORTS.map((port, i) => (
              <tr key={port.name} className="hover:bg-[#f8f9fa] transition-all cursor-pointer">
                <td className="px-6 py-4 font-medium text-[#9aa0a6]">{i+1}</td>
                <td className="px-6 py-4 font-bold text-[#202124]">{port.name}</td>
